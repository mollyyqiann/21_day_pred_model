...
**The 21-day window survived a direct challenge.** When the live hold cap moved to 26 days, a y26 retrain (same rows, same split, same recipe, only the label window changed) was tested on 2026-09-22 — and **rejected**: on the 26-day outcome itself, the y21 model out-picked the y26-trained one (P@5 23.0% vs 17.2%, +5.9pp, bootstrap P≈100% with an overlap caveat). The labels agree on 99.2% of rows, yet the two models' daily top-5 overlapped only 51.7% — the head of this ranking is a cluster of near-ties, hypersensitive to small objective changes, and the extra day-22-to-26 "slow burn" positives appear to dilute the fast-touch gradient that top-5 selection depends on. Label windows and holding rules are separate decisions; here the mismatch (21-day label, 26-day hold) measurably beats alignment.
...
**Bad data stands the day down — six layers deep.** Added 2026-09-23 after a mid-download yfinance rate-limit crash left a 60-row score file (of ~501) that was nearly traded on, and its post-mortem exposed a second, independent bug: the fetch pulled `period="120d"` — exactly 120 trading bars — while `ma60_slope_60d` (a 60-bar MA shifted 60 bars) first becomes valid at bar 120, so that feature only ever existed on the *final* bar and any ticker missing a single bar silently vanished from "candidates with full features". The layers, any one of which would have prevented the incident: (1) the fetch is **chunked** (60 tickers/batch, 12s pause, `MG_FETCH_CHUNK`/`MG_FETCH_PAUSE`) and **260 days deep** (~110 bars of margin for the slope); (2) a **coverage floor** — below 80% fetched the scorer exits non-zero and writes nothing, and both its writes are atomic (tmp+replace), so a partial file can never again masquerade as a complete one; (3) the 15:45 job **retries the fetch** up to 3× with 45s backoff before giving up; (4) a **data gate** — under 400 rows (`MG_MIN_UNIVERSE`) or a stale asof, no plan is written and no counter mutates, and a held name missing from the file accrues **no** dropout days (missing ≠ out-of-rank — before this fix two healthy holdings were one bad day from being sold over an outage); (5) the executor's three guards (calendar, plan date, plan asof); (6) the wrapper's post-run audit, below.
...
Two lessons from a silent failure on 2026-09-14, when a session placed 4 real orders and then had its bookkeeping blocked, exiting rc=0 with the fills recorded nowhere: **`Bash(prefix:*)` permission rules do not match commands containing newlines**, so every command the session runs must be a single line (compact JSON, one-line receipts — the prompt now enforces this); and a headless session cannot be trusted to report its own failure to report, so the **wrapper audits it from plain bash** afterwards — if the plan had orders but the position state was not reconciled today, or the session output mentions a permission denial, the wrapper itself sends the alert.

**Encoded rules** (as of 2026-09-21). Entry: top-5 by `raw_margin`, extension indicator FAVOURABLE (0 of 3 flags), free slot, and past the re-entry gates — a 30-day wash-sale block after a losing exit plus a 60-day cooldown after any exit (re-buying a recent exit measured −9.3pp vs a first entry; the original never-buy-a-name-twice rule was retired 2026-09-10). Exit: no take profit (`MG_TAKE_PROFIT` re-enables a half-out one), −15% stop loss, out of the top-15 for 2 consecutive publication days, or a 26-trading-day hold cap (raised from 21 on 2026-09-21: a cap sweep on the 31-month PIT replay put 21 at +$1,071 total, 26 at +$1,312, no cap at +$1,789 — longer is better because the cap's only real effect is amputating the right tail, but no-cap is carried by three multi-month rides and loses money in choppy 2024, so the user took the one-step widening; the model's 21-day *target* window is unchanged). Stops are checked once daily at 15:45 against the same-session price (a gap through the level exits at the 15:55 price, not at the level) and are exempt from the 2-day minimum hold. Sizing: 8 equal slots, fractional dollar orders.

**Why a stop loss when the dropout rule exists:** they catch opposite failure modes. `raw_margin` correlates −0.71 with `dd_60d`, so a position that falls hard ranks *higher* and re-entrenches in the top-15 — the "model changed its mind" exit is structurally unable to fire on a crasher, and only the stop catches those. What live trading then showed is the other direction: dropout fires readily on positions that go *quiet* or get displaced by new volatility leaders, and it produced every realized exit of the strategy's first live month (at +10.7% as often as at −5.3% — with no take profit it is also the de-facto profit-taker). The −15% stop level is sized from the book (top-15 median daily ATR 5.3%, so −15% ≈ 2.8× ATR — inside the classic 2–4× band), not fitted to outcomes; `MG_STOP_LOSS` overrides it, `0` disables.

**The take-profit dispute was adjudicated — the answer was to have none.** Two analyses spent months disagreeing (+12% vs +30%, each with a backtest behind it). [`code/136_tp_adjudication.py`](code/136_tp_adjudication.py) showed they were measuring different populations: 43% of the rows behind the +12% case were 9th-or-later re-appearances of a name (mean 21d −10.6%) that the live rules can never buy, and on the 34 tradeable first appearances every *full-size* take-profit lost to holding — the real variable was size, not level (half-out dominated full-out at every level in both populations). [`code/137_pit_replay.py`](code/137_pit_replay.py) then replayed the whole rule set over 31 months of point-in-time data and found **no take profit beats every level of one on return, volatility and drawdown together**, so the live book has had none since 2026-09-10 (`MG_TAKE_PROFIT` re-enables a half-out at the given level). [`code/126_mg_shadow_b.py`](code/126_mg_shadow_b.py) still books the retired rule daily beside the live book, so the decision keeps being re-tested on live data instead of frozen. If you take one thing from this repo, take the population split in 136 — it is why the two backtests disagreed.

**Exits were unreachable until 2026-09.** `TAKE_PROFIT` was declared, printed in every plan footer, and never compared against a price; separately, the dropout rule cannot fire on these names because the model re-picks its own holdings, so they never leave the top-15. That left the 21-day cap as the only working exit — the strategy could accumulate but never sell on merit. Worth knowing if you are reading the rule list and assuming it runs.
...
│   ├── 126_mg_shadow_b.py                   ← daily shadow ledger of the retired TP rule ★
│   ├── 135/136/137_*.py                     ← rule adjudication: TP levers, population split,
│   │                                          31-month point-in-time replay
