...
**Why a stop loss when the dropout rule exists:** it can't do the job. `raw_margin` correlates −0.71 with `dd_60d`, so a position that falls hard ranks *higher* and re-entrenches in the top-15 — 13 of a recent day's top-15 sat more than 25% below their 60-day highs. The "model changed its mind" exit is structurally unable to fire on a loser, which had left "went up 12%" and "21 days passed" as the only exits. The −15% level is sized from the book (top-15 median daily ATR 5.3%, so −15% ≈ 2.8× ATR — inside the classic 2–4× band) and is **unbacktested**; `MG_STOP_LOSS` overrides it, `0` disables.

**The take-profit level is contested, and the repo does not resolve it.** Two analyses disagree: one found TP30 > TP20 > TP12 under every realistic fill from 09:30 to 15:55, arguing the +12% result was an artifact of an unrealistic same-close entry; the other found +30% fires on only ~11% of picks while the measured MFE of FAVOURABLE picks is a median +12.27%, peaking near day 14 and giving back ~10.4pp to the close. The live default is **+12%** (adopted 2026-09-09); `MG_TAKE_PROFIT=0.30` switches it back without editing anything. If you take one thing from this repo, take the disagreement rather than either number.
